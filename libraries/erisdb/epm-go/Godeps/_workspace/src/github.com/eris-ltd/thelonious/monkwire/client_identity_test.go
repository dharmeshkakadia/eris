...
func TestClientIdentity(t *testing.T) {
	...
	expected := fmt.Sprintf("Thelonious(G)/v0.5.16/test/%s/%s", runtime.GOOS, runtime.Version())
	...
	clientString = clientIdentity.String()
	expected = fmt.Sprintf("Thelonious(G)/v0.5.16/test2/%s/%s", runtime.GOOS, runtime.Version())
	if clientString != expected {
		t.Errorf("Expected clientIdentity to be %v, got %v", expected, clientString)
	}
}
