...
func ParanoiaCheck(t1 *Trie) (bool, *Trie) {
	t2 := New(monkutil.Config.Db, "")

	t1.NewIterator().Each(func(key string, v *monkutil.Value) {
		t2.Update(key, v.Str())
	})

	a := monkutil.NewValue(t2.Root).Bytes()
	b := monkutil.NewValue(t1.Root).Bytes()
...
type Node struct {
	Key   []byte
	Value *monkutil.Value
	Dirty bool
}

func NewNode(key []byte, val *monkutil.Value, dirty bool) *Node {
...
func (cache *Cache) Get(key []byte) *monkutil.Value {
	...
	value := monkutil.NewValueFromBytes(data)
...
func copyRoot(root interface{}) interface{} {
	...
	if b, ok := root.([]byte); ok {
		prevRootCopy = monkutil.CopyBytes(b)
	} else {
		prevRootCopy = root
	}
...
func New(db monkutil.Database, Root interface{}) *Trie {
...
func (t *Trie) Get(key string) string {
	...
	k := CompactHexDecode(key)
	c := monkutil.NewValue(t.getState(t.Root, k))
...
func (t *Trie) Cmp(trie *Trie) bool {
	return monkutil.NewValue(t.Root).Cmp(monkutil.NewValue(trie.Root))
}
...
func (t *Trie) getNode(node interface{}) *monkutil.Value {
	n := monkutil.NewValue(node)
	...
	if len(str) == 0 {
		...
		return monkutil.NewValueFromBytes([]byte(str))
	}
...
func (it *TrieIterator) workNode(currentNode *monkutil.Value) {
	if currentNode.Len() == 2 {
		k := CompactDecode(currentNode.Get(0).Str())
...
func (it *TrieIterator) Collect() [][]byte {
	...
	it.getNode(monkutil.NewValue(it.trie.Root).Bytes())
...
type EachCallback func(key string, node *monkutil.Value)

func (it *TrieIterator) Each(cb EachCallback) {
	it.fetchNode(nil, monkutil.NewValue(it.trie.Root).Bytes(), cb)
}
...
func (it *TrieIterator) iterateNode(key []int, currentNode *monkutil.Value, cb EachCallback) {
	if currentNode.Len() == 2 {
		k := CompactDecode(currentNode.Get(0).Str())
		pk := append(key, k...)
