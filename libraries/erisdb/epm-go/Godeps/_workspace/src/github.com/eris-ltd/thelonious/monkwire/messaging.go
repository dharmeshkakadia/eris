...
func ReadMessages(conn net.Conn) (msgs []*Msg, err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("monkwire.ReadMessage error: %v", r)
		}
	}()
	...
	for {
		...
		if msgLength == 0 {
			...
			msgLength = int(monkutil.BytesToNumber(buff[4:8]))
	...
	for _, m := range messages {
		decoder := monkutil.NewValueFromBytes(m)
		// Type of message
...
func WriteMessage(conn net.Conn, msg *Msg) error {
	...
	encoded := monkutil.NewValue(append([]interface{}{byte(msg.Type)}, msg.Data.Slice()...)).Encode()
	payloadLength := monkutil.NumberToBytes(uint32(len(encoded)), 32)
