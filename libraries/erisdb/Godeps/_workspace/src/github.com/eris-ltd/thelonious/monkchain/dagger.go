...
var powlogger = monklog.NewLogger("POW")

type PoW interface {
	Search(block *Block, reactChan chan monkreact.Event) []byte
...
func (pow *EasyPow) Search(block *Block, reactChan chan monkreact.Event) []byte {
	...
	for {
		...
		default:
			...
			sha := monkcrypto.Sha3Bin(big.NewInt(r.Int63()).Bytes())
