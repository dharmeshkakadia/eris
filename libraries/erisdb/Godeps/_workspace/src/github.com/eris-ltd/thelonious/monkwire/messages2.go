...
func (self *Connection) Write(typ MsgType, v ...interface{}) error {
	...
	for _, value := range v {
		if encodable, ok := value.(monkutil.RlpEncodeDecode); ok {
	...
	encoded := monkutil.NewValue(slice).Encode()
...
func (self *Connection) readMessage(data []byte) (msg *Msg, remaining []byte, done bool, err error) {
	...
	messageLength := monkutil.BytesToNumber(data[4:8])
	...
	decoder := monkutil.NewValueFromBytes(message)
	// Type of message
