...
type Filter struct {
	...
	MessageCallback func(monkstate.Messages)
}
...
func NewFilterFromMap(object map[string]interface{}, eth NodeManager) *Filter {
	...
	if object["to"] != nil {
		...
		filter.AddTo(monkutil.Hex2Bytes(val.Str()))
	}

	if object["from"] != nil {
		val := monkutil.NewValue(object["from"])
		filter.AddFrom(monkutil.Hex2Bytes(val.Str()))
	}
...
func (self *Filter) FilterMessages(msgs []*monkstate.Message) []*monkstate.Message {
	var messages []*monkstate.Message
...
func mapToData(m map[string]interface{}) (d data) {
	if str, ok := m["id"].(string); ok {
		d.id = monkutil.Hex2Bytes(str)
	}

	if str, ok := m["at"].(string); ok {
		d.address = monkutil.Hex2Bytes(str)
	}
...
func makeAltered(v interface{}) (d []data) {
	if str, ok := v.(string); ok {
		d = append(d, data{monkutil.Hex2Bytes(str), nil})
