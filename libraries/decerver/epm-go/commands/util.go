...
func makeRPCDir(r string) error {
	last := filepath.Base(r)
	if last != "rpc" {
		r = path.Join(r, "rpc")
	}
	// create rpc dir and copy config
	if _, err := os.Stat(r); err != nil {
		if err := os.Mkdir(path.Join(r), 0700); err != nil {
			return err
		}
	}

	// copy config
	if _, err := os.Stat(path.Join(r, "config.json")); err != nil {
		// get the chain type
		sp := strings.Split(r, "/")
		chainType := ""
		for i, s := range sp {
		...
		if chainType == "" {
			return fmt.Errorf("could not resolve chain type from root %s", r)
		} else if chainType == "ethereum" {
			// no rpc yet...
			return nil
		}

		chain := mod.NewChain(chainType, true)
		err := chain.ReadConfig(path.Join(r, "..", "config.json"))
		if err != nil {
			return err
		}
		chain.SetProperty("RootDir", r)
		err = chain.WriteConfig(path.Join(r, "config.json"))
		if err != nil {
			return err
		}
	}
