...
func NewTransactionFromValue(val *monkutil.Value) *Transaction {
	tx := &Transaction{}
...
func (tx *Transaction) Hash() []byte {
	...
	return monkcrypto.Sha3Bin(monkutil.NewValue(data).Encode())
}
...
func (tx *Transaction) CreationAddress() []byte {
	return monkcrypto.Sha3Bin(monkutil.NewValue([]interface{}{tx.Sender(), tx.Nonce}).Encode())[12:]
}
...
func (tx *Transaction) RlpValue() *monkutil.Value {
	return monkutil.NewValue(tx.RlpData())
}
...
func (tx *Transaction) RlpDecode(data []byte) {
	tx.RlpValueDecode(monkutil.NewValueFromBytes(data))
}
